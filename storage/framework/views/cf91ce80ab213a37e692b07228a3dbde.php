...
<div class="max-w-7xl mx-auto space-y-6 pb-12 relative z-10 w-full px-4 sm:px-0 mt-4 md:mt-0">
    ...
    <!-- Main Content Grid -->
    <form id="edit-product-form" action="<?php echo e(route('admin.produk.update', $product)); ?>" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <?php echo csrf_field(); ?>
                    ...
                    <div class="md:col-span-1">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="price_display"
                                ...
                                value="<?php echo e(old('price', $product->price > 0 ? number_format($product->price, 0, ',', '.') : '')); ?>"
                                placeholder="0"
                                required>
                            <input type="hidden" id="price" name="price" value="<?php echo e(old('price', $product->price)); ?>">
                        </div>
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-3">
                            <?php $__currentLoopData = $existingImages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="relative aspect-square rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
                                    <img src="<?php echo e($image); ?>" alt="Gambar <?php echo e($index + 1); ?>" class="w-full h-full object-cover">
                                    <div class="absolute top-2 left-2 px-2 py-1 rounded bg-black/70 text-white text-[10px] font-bold"><?php echo e($index === 0 ? 'Utama' : $index + 1); ?></div>
                                </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    <?php endif; ?>
    ...
    <form id="delete-image-form-<?php echo e($product->id); ?>" action="<?php echo e(route('admin.produk.image.delete', $product)); ?>" method="POST" class="hidden">
        <?php echo csrf_field(); ?>
        <?php echo method_field('DELETE'); ?>
    </form>

    <form id="delete-product-form-<?php echo e($product->id); ?>" action="<?php echo e(route('admin.produk.destroy', $product)); ?>" method="POST" class="hidden">
        <?php echo csrf_field(); ?>
        <?php echo method_field('DELETE'); ?>
    </form>
    ...
    function submitDeleteImageForm() {
        if (confirm('Apakah Anda yakin ingin menghapus gambar produk ini?')) {
            const deleteForm = document.getElementById('delete-image-form-<?php echo e($product->id); ?>');
            if (deleteForm) {
                deleteForm.submit();
            }
        }
    ...
    function submitDeleteProductForm() {
        if (confirm('Apakah Anda yakin ingin menghapus produk ini secara permanen?')) {
            const deleteForm = document.getElementById('delete-product-form-<?php echo e($product->id); ?>');
            if (deleteForm) {
                deleteForm.submit();
            }
        }
