...
<div class="max-w-7xl mx-auto space-y-6 pb-12 relative z-10 w-full px-4 sm:px-0 mt-4 md:mt-0">
    ...
    <div class="card p-4 sm:p-5 flex flex-col sm:flex-row gap-4 justify-between items-center w-full">
        ...
        <a href="<?php echo e(route('admin.produk.create')); ?>" class="btn-primary w-full sm:w-auto text-sm justify-center shadow-md">
    ...
    <div class="card overflow-hidden border-primary-100">
        ...
        <div class="px-6 py-3 border-b border-primary-100 flex flex-wrap gap-4 items-center justify-between bg-white">
            ...
            <span class="text-xs text-gray-500 font-medium">Menampilkan <?php echo e($products->firstItem() ?? 1); ?>-<?php echo e($products->lastItem() ?? $products->count()); ?> dari <?php echo e($products->total()); ?> produk</span>
        </div>
            ...
            <table class="w-full text-left text-sm whitespace-nowrap lg:whitespace-normal">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-primary-50/10 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="w-14 h-14 bg-gray-50 border border-gray-100 rounded-lg flex items-center justify-center p-1.5 shrink-0">
                                <?php if($product->getFirstImage()): ?>
                                    <img loading="lazy" src="<?php echo e($product->getFirstImage()); ?>" alt="<?php echo e($product->name); ?>" class="w-full h-full object-cover rounded mix-blend-multiply">
                                <?php else: ?>
                        ...
                        <td class="px-6 py-4">
                            <div class="font-bold text-gray-900 group-hover:text-primary-600 transition-colors"><?php echo e($product->name); ?></div>
                            <div class="text-xs text-gray-500 flex items-center gap-1 mt-0.5"><i class="ph ph-tag w-3 h-3 text-gray-400"></i> SKU: <?php echo e($product->sku); ?></div>
                        </td>
                        <td class="px-6 py-4">
                            <?php if($product->category): ?>
                                <span class="inline-flex px-2 py-1 text-xs font-medium rounded-md bg-green-50 text-green-700 border border-green-200"><?php echo e($product->category->name); ?></span>
                            <?php else: ?>
                                <span class="inline-flex px-2 py-1 text-xs font-medium rounded-md bg-gray-50 text-gray-700 border border-gray-200">-</span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 text-gray-900 font-bold"><?php echo e($product->getFormattedPrice()); ?><span class="text-gray-500 font-normal text-xs">/<?php echo e($product->unit ?? 'unit'); ?></span></td>
                        <td class="px-6 py-4 text-center">
                            <?php if($product->stock < 10): ?>
                                <span class="inline-flex px-2.5 py-1 text-xs font-bold rounded-full bg-red-50 text-red-700 border border-red-200"><?php echo e($product->stock); ?></span>
                            <?php else: ?>
                                <span class="inline-flex px-2.5 py-1 text-xs font-bold rounded-full bg-primary-50 text-primary-700 border border-primary-200"><?php echo e($product->stock); ?></span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="<?php echo e(route('admin.produk.show', $product)); ?>" class="icon-button text-gray-500 hover:text-blue-600 hover:bg-blue-50 bg-white border border-gray-200 rounded-lg shadow-sm transition-colors" title="Lihat Detail">
                                    <i class="ph ph-eye ph-bold w-4 h-4"></i>
                                </a>
                                <a href="<?php echo e(route('admin.produk.edit', $product)); ?>" class="icon-button text-gray-500 hover:text-amber-600 hover:bg-amber-50 bg-white border border-gray-200 rounded-lg shadow-sm transition-colors" title="Edit">
                                    <i class="ph ph-pencil-simple ph-bold w-4 h-4"></i>
                                </a>
                                <form action="<?php echo e(route('admin.produk.destroy', $product)); ?>" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini?');">
                                    <?php echo csrf_field(); ?>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <a href="<?php echo e(route('admin.produk.index')); ?>" class="text-primary-600 hover:underline text-sm">Reset filter</a>
                            </div>
                        </td>
                    </tr>
                    <?php endif; ?>
        ...
        <div class="bg-gradient-to-r from-primary-50/30 to-primary-50/10 border-t border-primary-100 p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span class="text-sm text-gray-500 font-medium">Menampilkan <span class="text-gray-900 font-bold"><?php echo e($products->firstItem() ?? 1); ?>-<?php echo e($products->lastItem() ?? $products->count()); ?></span> dari <span class="text-gray-900 font-bold"><?php echo e($products->total()); ?></span> Produk</span>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\bintang-agung-tani\resources\views/admin/kelola-produk.blade.php ENDPATH**/ ?>