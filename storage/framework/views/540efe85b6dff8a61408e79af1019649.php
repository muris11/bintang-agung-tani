        ...
        <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-primary-50/50 to-transparent">
            <div class="flex items-center gap-3">
                <div class="w-12 h-12 rounded-xl bg-primary-100 text-primary-600 flex items-center justify-center">
                    <i class="ph <?php echo e($category->icon ?? 'ph-tag'); ?> ph-duotone w-6 h-6"></i>
                </div>
                <div>
                    <h2 class="text-lg font-bold text-gray-900"><?php echo e($category->name); ?></h2>
                    <p class="text-sm text-gray-500">ID: #<?php echo e($category->id); ?></p>
                </div>
            </div>
        </div>

        <form action="<?php echo e(route('admin.kategori.update', $category)); ?>" method="POST" class="p-6 space-y-6">
            <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>
            ...
            <div x-data="{ open: false, selected: '<?php echo e(old('icon', $category->icon) ?? 'ph-tag'); ?>', search: '' }" 
                ...
                <div class="relative">
                    <button type="button" 
                            @click="open = !open"
                            class="form-input w-full flex items-center gap-3 text-left bg-white hover:bg-gray-50 transition-colors <?php $__errorArgs = ['icon'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
...
<div x-show="open" x-cloak
                        ...
                        <div class="overflow-y-auto max-h-64 p-3">
                            <div class="grid grid-cols-6 gap-2">
                                ...
                                <?php $__currentLoopData = $icons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $icon): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <button type="button"
                                            @click="selected = '<?php echo e($icon); ?>'; open = false"
                                            x-show="!search || '<?php echo e($icon); ?>'.toLowerCase().includes(search.toLowerCase())"
                                            class="flex flex-col items-center justify-center p-2 rounded-lg hover:bg-primary-50 transition-colors group"
                                            :class="{ 'bg-primary-100 ring-2 ring-primary-500': selected === '<?php echo e($icon); ?>' }">
                                        <i class="ph <?php echo e($icon); ?> ph-duotone w-6 h-6 text-gray-600 group-hover:text-primary-600 mb-1"></i>
                                        <span class="text-[10px] text-gray-500 truncate w-full text-center"><?php echo e(str_replace('ph-', '', $icon)); ?></span>
                                    </button>
                ...
                <input type="hidden" name="icon" x-model="selected" value="<?php echo e(old('icon', $category->icon)); ?>">
...
unset($__errorArgs, $__bag); ?>
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          class="form-input w-full resize-none <?php $__errorArgs = ['description'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 focus:border-red-500 focus:ring-red-200 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
            ...
            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" <?php echo e(old('is_active', $category->is_active) ? 'checked' : ''); ?>>
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-gray-100">
                <a href="<?php echo e(route('admin.kategori.index')); ?>" class="btn-secondary justify-center text-center">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\bintang-agung-tani\resources\views\admin\edit-kategori.blade.php ENDPATH**/ ?>