<?php $__env->startSection('title', 'Verifikasi Bukti Pembayaran'); ?>
...
<div class="max-w-7xl mx-auto space-y-6 pb-12 relative z-10 w-full px-4 sm:px-0 mt-4 md:mt-0">
        ...
        <div class="card p-4 hover:-translate-y-1 transition-transform cursor-pointer border-b-4 border-b-blue-500 bg-blue-50/10">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-black text-gray-900"><?php echo e($paymentProofs->total()); ?></h3>
                </div>
        ...
        <div class="card p-4 hover:-translate-y-1 transition-transform cursor-pointer border-b-4 border-b-amber-500 bg-amber-50/10">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-black text-gray-900"><?php echo e($paymentProofs->where('status', 'pending')->count()); ?></h3>
                </div>
        ...
        <div class="card p-4 hover:-translate-y-1 transition-transform cursor-pointer border-b-4 border-b-emerald-500 bg-emerald-50/10">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-black text-gray-900"><?php echo e($paymentProofs->where('status', 'verified')->count()); ?></h3>
                </div>
        ...
        <div class="card p-4 hover:-translate-y-1 transition-transform cursor-pointer border-b-4 border-b-red-500 bg-red-50/10">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-black text-gray-900"><?php echo e($paymentProofs->where('status', 'rejected')->count()); ?></h3>
                </div>
    ...
    <div class="card p-6 border-primary-100">
        <form action="<?php echo e(route('admin.payment-proofs.index')); ?>" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
            <div class="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="date_from" value="<?php echo e(request('date_from')); ?>" class="form-input w-full">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Sampai Tanggal</label>
                    <input type="date" name="date_to" value="<?php echo e(request('date_to')); ?>" class="form-input w-full">
                </div>
                <div>
            ...
            <div class="flex gap-2">
                ...
                <a href="<?php echo e(route('admin.payment-proofs.index')); ?>" class="btn-secondary h-11 px-4 whitespace-nowrap">
    ...
    <div class="card p-0 overflow-hidden w-full border-primary-100">
        ...
        <div class="table-responsive">
            <table class="w-full text-left text-sm whitespace-nowrap" role="table" aria-label="Daftar bukti pembayaran">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    <?php $__empty_1 = true; $__currentLoopData = $paymentProofs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $proof): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-primary-50/30 transition-colors">
                        <td class="px-6 py-4">
                            <span class="font-mono font-bold text-primary-600 bg-primary-50 px-2 py-1 rounded border border-primary-100"><?php echo e($proof->order->order_number ?? '-'); ?></span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center font-bold text-xs shrink-0">
                                    <?php echo e(substr($proof->user->name ?? 'U', 0, 1)); ?>

                                </div>
                                <span class="font-medium text-gray-900"><?php echo e($proof->user->name ?? '-'); ?></span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-gray-700"><?php echo e($proof->paymentMethod->name ?? '-'); ?></td>
                        <td class="px-6 py-4">
                            <span class="text-sm font-medium text-gray-900"><?php echo e($proof->created_at->format('d M Y')); ?></span>
                            <span class="text-xs text-gray-500 block"><?php echo e($proof->created_at->format('H:i')); ?> WIB</span>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <a href="<?php echo e(route('admin.payment-proofs.show', $proof)); ?>" class="btn-primary text-xs h-8 px-3 inline-flex items-center gap-1.5">
        ...
        <!-- Pagination -->
        <?php if($paymentProofs->hasPages()): ?>
        <div class="px-6 py-4 border-t border-gray-100">
            <?php echo e($paymentProofs->links()); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\bintang-agung-tani\resources\views\admin\payment-proofs\index.blade.php ENDPATH**/ ?>