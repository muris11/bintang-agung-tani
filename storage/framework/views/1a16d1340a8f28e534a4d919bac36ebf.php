...
<div class="max-w-7xl mx-auto space-y-6 pb-12 relative z-10">
            ...
            <nav class="flex text-sm text-gray-500 mb-2" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    <li class="inline-flex items-center">
                        <a href="<?php echo e(route('user.dashboard')); ?>" class="hover:text-gray-800 transition-colors">Dashboard</a>
                    </li>
                    <li>
    ...
    <div class="border-b border-gray-200">
        <ul class="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 overflow-x-auto scrollbar-hide">
            <li class="me-2">
                <a href="<?php echo e(route('user.orders.index', ['status' => 'semua'])); ?>" class="inline-flex items-center justify-center p-4 <?php echo e($currentStatus === 'semua' ? 'text-primary-600 border-b-2 border-primary-600 rounded-t-lg active bg-primary-50/50' : 'border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 transition-all'); ?>">
                    Semua Pesanan
                </a>
            </li>
            <li class="me-2">
                <a href="<?php echo e(route('user.orders.index', ['status' => 'menunggu-verifikasi'])); ?>" class="inline-flex items-center justify-center p-4 <?php echo e($currentStatus === 'menunggu-verifikasi' ? 'text-primary-600 border-b-2 border-primary-600 rounded-t-lg active bg-primary-50/50' : 'border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 transition-all'); ?>">
                    Menunggu Verifikasi
                </a>
            </li>
            <li class="me-2">
                <a href="<?php echo e(route('user.orders.index', ['status' => 'diproses'])); ?>" class="inline-flex items-center justify-center p-4 <?php echo e($currentStatus === 'diproses' ? 'text-primary-600 border-b-2 border-primary-600 rounded-t-lg active bg-primary-50/50' : 'border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 transition-all'); ?>">
                    Diproses
                </a>
            </li>
            <li class="me-2">
                <a href="<?php echo e(route('user.orders.index', ['status' => 'selesai'])); ?>" class="inline-flex items-center justify-center p-4 <?php echo e($currentStatus === 'selesai' ? 'text-primary-600 border-b-2 border-primary-600 rounded-t-lg active bg-primary-50/50' : 'border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 transition-all'); ?>">
                    Selesai
                </a>
            </li>
            <li class="me-2">
                <a href="<?php echo e(route('user.orders.index', ['status' => 'dibatalkan'])); ?>" class="inline-flex items-center justify-center p-4 <?php echo e($currentStatus === 'dibatalkan' ? 'text-primary-600 border-b-2 border-primary-600 rounded-t-lg active bg-primary-50/50' : 'border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 transition-all'); ?>">
                    Dibatalkan
                </a>
            </li>
        </ul>
    ...
    <div class="card overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm whitespace-nowrap md:whitespace-normal">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    ...
                    <!-- Row <?php echo e($loop->iteration); ?> -->
                    <tr class="hover:bg-primary-50/10 transition-colors">
                        <td class="px-6 py-5 align-top">
                            <a href="<?php echo e(route('user.orders.show', $order)); ?>" class="font-bold text-gray-900 hover:text-primary-600 block mb-3 hover:underline"><?php echo e($order->order_number); ?></a>
                            ...
                            <div class="flex items-start gap-3 w-56 sm:w-auto">
                                <div class="w-12 h-12 shrink-0 bg-gray-50 border border-gray-100 rounded-lg p-1.5 flex items-center justify-center">
                                    <?php if($order->items->first()->product && $order->items->first()->product->getFirstImage()): ?>
                                        <img loading="lazy" src="<?php echo e($order->items->first()->product->getFirstImage()); ?>" class="w-full h-full object-cover rounded mix-blend-multiply">
                                    <?php else: ?>
                                        <i class="ph ph-package w-6 h-6 text-gray-300"></i>
                                    <?php endif; ?>
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-xs font-bold text-gray-900 leading-tight"><?php echo e($order->items->first()->product_name); ?></span>
                                    <span class="text-[10px] text-gray-500 flex items-center gap-1 mt-0.5"><i class="ph ph-package w-3 h-3 text-gray-400"></i> <?php echo e($order->items->first()->quantity); ?> <?php echo e($order->items->first()->unit); ?></span>
                                    <?php if($order->items->count() > 1): ?>
                                    <span class="text-xs text-primary-600 font-semibold mt-1">+ <?php echo e($order->items->count() - 1); ?> Produk Lainnya</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-5 align-top text-gray-600 font-medium"><?php echo e($order->created_at->format('d F Y')); ?></td>
                        <td class="px-6 py-5 align-top">
                            <span class="inline-flex py-1 px-3 text-xs font-bold rounded-full <?php echo e($order->getStatusBadgeClass()); ?> border">
                                <?php echo e($order->getStatusLabel()); ?>

                            </span>
                        </td>
                        <td class="px-6 py-5 align-top text-right">
                            <div class="font-bold text-gray-900 text-base"><?php echo e($order->getFormattedTotal()); ?></div>
                            ...
                            <?php elseif($order->canBePaid()): ?>
                            <a href="<?php echo e(route('user.payments.select-method', $order)); ?>" class="btn-primary mt-3 text-xs py-2 px-3 inline-flex items-center gap-1">
                                Bayar Sekarang
                                <i class="ph ph-arrow-right ph-bold"></i>
                            </a>
                            <?php elseif($order->isProcessing() || $order->isMenungguVerifikasi()): ?>
                            <a href="<?php echo e(route('user.orders.show', $order)); ?>" class="btn-secondary mt-3 text-xs py-2 px-3 inline-block">
                                Pantau Pesanan
                            </a>
                            <?php else: ?>
                            <a href="<?php echo e(route('user.orders.show', $order)); ?>" class="btn-secondary mt-3 text-xs py-2 px-3 inline-block">
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center text-gray-500">
                            ...
                            <a href="<?php echo e(route('user.produk.index')); ?>" class="btn-primary mt-4 inline-flex items-center gap-2">
        ...
        <div class="bg-gray-50/50 border-t border-gray-100 p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <span class="text-sm text-gray-500 font-medium">Menampilkan <?php echo e($orders->firstItem()); ?>-<?php echo e($orders->lastItem()); ?> dari <?php echo e($orders->total()); ?> pesanan</span>
            <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\bintang-agung-tani\resources\views\user\riwayat.blade.php ENDPATH**/ ?>