<?php $__env->startSection('title', 'Detail Bukti Verifikasi'); ?>
...
<div class="max-w-7xl mx-auto space-y-6 pb-12 relative z-10 w-full px-4 sm:px-0 mt-4 md:mt-0" x-data="{ showImageModal: false }">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
        <div>
            ...
            <div class="flex items-center gap-3">
                ...
                <span class="inline-flex px-2.5 py-1 text-xs font-bold rounded-full <?php echo e($payment->isPending() ? 'bg-orange-100 text-orange-700 border border-orange-200' : ($payment->isVerified() ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200')); ?> mt-1.5 shadow-sm">
                    <?php echo e($payment->isPending() ? 'Menunggu Verifikasi' : ($payment->isVerified() ? 'Terverifikasi' : 'Ditolak')); ?>
        ...
        <div class="flex items-center gap-3">
            <a href="<?php echo e(route('admin.verifikasi.index')); ?>" class="btn-secondary text-sm h-10 shadow-sm border-gray-200">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div class="space-y-6">
            <div class="card p-0 overflow-hidden border-t-4 border-t-<?php echo e($payment->isPending() ? 'orange' : ($payment->isVerified() ? 'green' : 'red')); ?>-500">
                <div class="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="<?php echo e($payment->getImageUrl()); ?>" download class="btn-secondary h-8 px-3 py-0 text-xs shadow-sm bg-white"><i class="ph ph-download-simple w-3.5 h-3.5 mr-1"></i> Unduh</a>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="p-6">
                    ...
                    <div class="relative w-full rounded-xl overflow-hidden bg-gray-100 border border-gray-200 flex items-center justify-center" style="min-height: 400px; max-height: 600px;">
                        <?php if($payment->image_path): ?>
                            <img loading="lazy" src="<?php echo e($payment->getImageUrl()); ?>" alt="Bukti Transfer" class="max-w-full max-h-[600px] object-contain shadow-sm border border-gray-200">
                        <?php else: ?>
        ...
        <div class="space-y-6">
            ...
            <div class="card p-6">
                <h3 class="text-lg font-bold text-gray-900 mb-5 pb-3 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <a href="<?php echo e(route('admin.orders.show', $payment->order)); ?>" class="text-xs font-semibold text-primary-600 flex items-center gap-1 hover:text-primary-700 bg-primary-50 px-2 py-1 rounded-md transition-colors">
                ...
                <div class="bg-gray-50 rounded-xl p-5 border border-gray-100 mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-black text-gray-900 leading-none"><?php echo e($payment->order->getFormattedTotal()); ?></p>
                    </div>
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        ...
                        <span class="font-bold text-gray-900"><?php echo e($payment->order->order_number ?? 'N/A'); ?></span>
                    </div>
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        <span class="text-gray-500 font-medium text-sm">Waktu Upload</span>
                        <span class="font-semibold text-gray-900 text-sm"><?php echo e($payment->created_at->format('d M Y, H:i')); ?> WIB</span>
                    </div>
                    <div class="flex justify-between items-start pb-3 border-b border-gray-100">
                        ...
                        <div class="text-right">
                            <span class="font-bold text-gray-900 block"><?php echo e($payment->user->name ?? 'N/A'); ?></span>
                            <span class="text-xs text-gray-500 mt-0.5 flex items-center justify-end gap-1"><i class="ph ph-envelope-simple w-3 h-3"></i> <?php echo e($payment->user->email ?? '-'); ?></span>
                        </div>
                    </div>
                    <div class="flex justify-between items-center pb-3 border-gray-100">
                        ...
                        <span class="font-bold text-gray-900 flex items-center gap-2 text-sm text-right">
                            <span class="px-2 py-0.5 rounded bg-blue-50 text-blue-700 text-xs font-black italic border border-blue-100"><?php echo e(strtoupper($payment->paymentMethod->code ?? $payment->paymentMethod->name ?? '-')); ?></span> Transfer Bank
                        </span>
                    </div>
                    ...
                    <div class="flex justify-between items-start pt-2 border-t border-gray-100">
                        ...
                        <span class="text-sm text-gray-700 text-right max-w-xs"><?php echo e($payment->notes); ?></span>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="card p-6 border-t-4 border-t-primary-500">
                ...
                <p class="text-sm text-gray-600 mb-6 font-medium leading-relaxed">
                    Pastikan dana <strong class="text-gray-900"><?php echo e($payment->order->getFormattedTotal()); ?></strong> sudah efektif masuk ke rekening mutasi Bank <?php echo e(strtoupper($payment->paymentMethod->name ?? $payment->paymentMethod->code ?? '')); ?> sebelum menyetujui. Tindakan ini tidak dapat dibatalkan dan akan langsung meneruskan pesanan ke tahap Pengepakan.
                </p>

                <form action="<?php echo e(route('admin.verifikasi.approve', $manualPayment ?? $payment)); ?>" method="POST" class="space-y-4">
                    <?php echo csrf_field(); ?>
                ...
                <form action="<?php echo e(route('admin.verifikasi.reject', $manualPayment ?? $payment)); ?>" method="POST" class="pt-4 border-t border-gray-100 border-dashed">
                    <?php echo csrf_field(); ?>
            ...
            <?php else: ?>
            <!-- Status Final -->
            <div class="card p-6 border-t-4 <?php echo e($payment->isVerified() ? 'border-t-green-500' : 'border-t-red-500'); ?>">
                <div class="flex items-center gap-2 mb-3">
                    <div class="w-8 h-8 rounded-lg <?php echo e($payment->isVerified() ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'); ?> flex items-center justify-center">
                        <i class="ph <?php echo e($payment->isVerified() ? 'ph-check-circle' : 'ph-x-circle'); ?> ph-bold w-4 h-4"></i>
                    </div>
                    <h2 class="text-lg font-bold text-gray-900">Status Verifikasi</h2>
                </div>

                <div class="p-4 rounded-xl <?php echo e($payment->isVerified() ? 'bg-green-50 border border-green-100' : 'bg-red-50 border border-red-100'); ?>">
                    <p class="text-sm <?php echo e($payment->isVerified() ? 'text-green-800' : 'text-red-800'); ?>">
                        ...
                        <?php else: ?>
                            ...
                            Alasan: <?php echo e($payment->admin_notes ?? 'Tidak ada alasan'); ?>
                ...
                <div class="mt-4 text-sm text-gray-500">
                    Diverifikasi oleh <span class="font-medium text-gray-700"><?php echo e($payment->verifier->name ?? 'Admin'); ?></span> pada <?php echo e($payment->verified_at->format('d M Y, H:i')); ?> WIB
                </div>
                <?php endif; ?>
            </div>
            <?php endif; ?>
    ...
    <div x-show="showImageModal" x-cloak style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/90 p-4"
        ...
        <div @click.away="showImageModal = false" class="relative max-w-4xl w-full flex items-center justify-center h-full"
            ...
            <?php if($payment->image_path): ?>
                <img loading="lazy" src="<?php echo e($payment->getImageUrl()); ?>" class="max-w-full max-h-full object-contain rounded shadow-2xl">
            <?php else: ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\bintang-agung-tani\resources\views\admin\detail-verifikasi.blade.php ENDPATH**/ ?>