...
<div class="max-w-7xl mx-auto space-y-6 pb-12 relative z-10 w-full px-4 sm:px-0 mt-4 md:mt-0" x-data="{ showDeleteModal: false, showAddModal: false, showEditModal: false, deleteTarget: '', editTarget: '' }">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <?php $__empty_1 = true; $__currentLoopData = $categories->take(4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <div class="card p-5 group flex flex-col justify-between hover:-translate-y-1 transition-transform duration-300">
            ...
            <div>
                <h3 class="text-3xl font-bold text-gray-900 mb-1"><?php echo e($category->products_count ?? $category->products->count() ?? 0); ?></h3>
                <p class="text-sm font-medium text-gray-500"><?php echo e($category->name); ?></p>
            </div>
        </div>
    ...
    <div class="card p-0 overflow-hidden w-full">
        ...
        <div class="p-5 border-b border-gray-100 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-white">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1"><?php echo e($categories->total()); ?> kategori tersedia</p>
            </div>
                    ...
                    <tr class="hover:bg-primary-50/30 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-lg bg-primary-50 text-primary-600 border border-primary-100 flex items-center justify-center shrink-0">
                                    <i class="ph <?php echo e($category->icon ?? 'ph-tag'); ?> ph-duotone w-5 h-5"></i>
                                </div>
                                <div>
                                    <span class="font-bold text-gray-900 block text-base leading-tight"><?php echo e($category->name); ?></span>
                                    <span class="text-xs text-gray-500 mt-0.5 block"><?php echo e($category->description ?? 'Kategori produk'); ?></span>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <span class="font-bold text-gray-900 text-base"><?php echo e($category->products_count ?? 0); ?></span>
                                <span class="text-xs text-gray-500">Item</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="<?php echo e(url('/admin/kategori/' . $category->id . '/edit')); ?>" class="px-3 py-1.5 text-xs font-semibold text-blue-700 bg-blue-50 border border-blue-100 border-b-2 hover:bg-blue-100 rounded-lg transition-colors flex items-center gap-1.5 focus:ring-2 focus:ring-blue-500/20 active:border-b active:translate-y-px">
                                    <i class="ph ph-pencil-simple ph-bold w-3.5 h-3.5"></i> Edit
                                </a>
                                <form action="<?php echo e(url('/admin/kategori/' . $category->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus kategori ini?')">
                                    <?php echo csrf_field(); ?>
        ...
        <div class="p-5 border-t border-gray-100 flex items-center justify-between text-sm bg-white">
            <span class="text-gray-500 font-medium">Menampilkan <span class="text-gray-900 font-bold"><?php echo e($categories->firstItem() ?? 0); ?>-<?php echo e($categories->lastItem() ?? 0); ?></span> dari <span class="text-gray-900 font-bold"><?php echo e($categories->total()); ?></span> kategori</span>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\bintang-agung-tani\resources\views/admin/kategori.blade.php ENDPATH**/ ?>