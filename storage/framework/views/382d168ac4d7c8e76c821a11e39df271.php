...
<div class="max-w-7xl mx-auto space-y-6 pb-12 relative z-10 w-full px-4 sm:px-0 mt-4 md:mt-0" x-data="{ showImageModal: false }">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
        <div>
            ...
            <div class="flex items-center gap-3">
                ...
                <span class="inline-flex px-2.5 py-1 text-xs font-bold rounded-full <?php echo e($payment->isPending() ? 'bg-orange-100 text-orange-700 border border-orange-200' : ($payment->isSuccess() ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200')); ?> mt-1.5 shadow-sm">
                    <?php echo e($payment->isPending() ? 'Review Tertunda' : ($payment->isSuccess() ? 'Terverifikasi' : 'Ditolak')); ?>
            ...
            <div class="card p-0 overflow-hidden border-t-4 border-t-orange-500">
                <div class="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="<?php echo e($payment->order && $payment->order->latestPaymentProof ? $payment->order->latestPaymentProof->getImageUrl() : '#'); ?>" download class="btn-secondary h-8 px-3 py-0 text-xs shadow-sm bg-white <?php echo e(!$payment->order || !$payment->order->latestPaymentProof ? 'opacity-50 cursor-not-allowed' : ''); ?>"><i class="ph ph-download-simple w-3.5 h-3.5 mr-1"></i> Unduh</a>
                    </div>
                </div>
                ...
                <div class="p-6">
                    ...
                    <div class="relative w-full rounded-xl overflow-hidden bg-gray-100 border border-gray-200 flex items-center justify-center" style="min-height: 400px; max-height: 600px;">
                        <?php if($payment->order && $payment->order->latestPaymentProof && $payment->order->latestPaymentProof->image_url): ?>
                            <img loading="lazy" src="<?php echo e($payment->order->latestPaymentProof->image_url); ?>" alt="Bukti Transfer" class="max-w-full max-h-[600px] object-contain shadow-sm border border-gray-200">
                        <?php else: ?>
        ...
        <div class="space-y-6">
            ...
            <div class="card p-6">
                ...
                <div class="bg-gray-50 rounded-xl p-5 border border-gray-100 mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-black text-gray-900 leading-none"><?php echo e($payment->getFormattedAmount()); ?></p>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center pb-3 border-gray-100">
                        ...
                        <span class="font-bold text-gray-900 flex items-center gap-2 text-sm text-right">
                            <span class="px-2 py-0.5 rounded bg-blue-50 text-blue-700 text-xs font-black italic border border-blue-100"><?php echo e(strtoupper($payment->payment_method)); ?></span> Transfer Bank
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="card p-6 border-t-4 border-t-primary-500">
                ...
                <p class="text-sm text-gray-600 mb-6 font-medium leading-relaxed">
                    Pastikan dana <strong class="text-gray-900"><?php echo e($payment->getFormattedAmount()); ?></strong> sudah efektif masuk ke rekening mutasi Bank <?php echo e(strtoupper($payment->payment_method)); ?> sebelum menyetujui. Tindakan ini tidak dapat dibatalkan dan akan langsung meneruskan pesanan ke tahap Pengepakan.
                </p>
                
                <form action="<?php echo e(route('admin.verifikasi.approve', $payment)); ?>" method="POST" class="space-y-4">
                    <?php echo csrf_field(); ?>
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button type="submit" class="btn-primary flex-1 h-12 shadow-md bg-green-600 hover:bg-green-700 hover:border-green-600 border-green-600 focus:ring-green-500/30" <?php echo e(!$payment->isPending() ? 'disabled' : ''); ?>>
                            <i class="ph ph-check-circle ph-bold w-5 h-5 mr-1.5"></i> Sesuai & Lanjutkan
                        </button>
                    </div>
                </form>
                    
                <form action="<?php echo e(route('admin.verifikasi.reject', $payment)); ?>" method="POST" class="pt-4 border-t border-gray-100 border-dashed">
                    <?php echo csrf_field(); ?>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <input type="text" name="reason" placeholder="Alasan penolakan (misal: Nominal kurang)" class="form-input flex-1 text-sm bg-gray-50/50" <?php echo e(!$payment->isPending() ? 'disabled' : ''); ?>>
                        <button type="submit" class="btn-primary h-10 shadow-md bg-red-600 hover:bg-red-700 hover:border-red-600 border-red-600 focus:ring-red-500/30 px-6 sm:w-auto w-full" <?php echo e(!$payment->isPending() ? 'disabled' : ''); ?>>
    ...
    <div x-show="showImageModal" x-cloak style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/90 p-4"
        ...
        <div @click.away="showImageModal = false" class="relative max-w-4xl w-full flex items-center justify-center h-full"
            ...
            <?php if($payment->order && $payment->order->latestPaymentProof && $payment->order->latestPaymentProof->image_url): ?>
                <img loading="lazy" src="<?php echo e($payment->order->latestPaymentProof->image_url); ?>" class="max-w-full max-h-full object-contain rounded shadow-2xl">
            <?php else: ?>
