...
<div class="max-w-7xl mx-auto space-y-6 pb-12 relative z-10 w-full px-4 sm:px-0 mt-4 md:mt-0">
    ...
    <div class="card p-6">
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h2 class="text-xl font-bold text-gray-900">{{ $product->name ?? 'Nama Produk' }}</h2>
                <p class="text-sm text-gray-500">SKU: {{ $product->sku ?? 'SKU-001' }}</p>
            </div>
        </div>

        <form action="{{ route('admin.stock.update', $product ?? 1) }}" method="POST" class="space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="p-4 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    <p class="text-2xl font-bold text-orange-600">{{ $product->min_stock ?? 10 }}</p>
                </div>
                <div class="p-4 bg-gray-50 rounded-xl border border-gray-200">
                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Status</p>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ ($product->stock ?? 0) > ($product->min_stock ?? 10) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ ($product->stock ?? 0) > ($product->min_stock ?? 10) ? 'Stok Aman' : 'Stok Menipis' }}
                    </span>
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 pt-6">
                ...
                <div class="mt-6 p-4 bg-blue-50 rounded-xl border border-blue-200">
                    ...
                    <p class="text-sm text-blue-700">Stok akan diupdate dari <strong>{{ $product->stock ?? 0 }}</strong> menjadi <strong id="new-stock-preview">-</strong></p>
                </div>
            </div>
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $log->type == 'add' ? 'bg-green-100 text-green-800' : ($log->type == 'subtract' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800') }}">
                                {{ $log->type == 'add' ? 'Tambah' : ($log->type == 'subtract' ? 'Kurangi' : 'Reset') }}
                            </span>
                        </td>
                        <td class="px-4 py-3 {{ $log->type == 'add' ? 'text-green-600' : ($log->type == 'subtract' ? 'text-red-600' : 'text-blue-600') }}">
                            {{ $log->type == 'add' ? '+' : ($log->type == 'subtract' ? '-' : '') }}{{ $log->quantity }}
                        </td>
                        <td class="px-4 py-3 font-medium">{{ $log->stock_after }}</td>
