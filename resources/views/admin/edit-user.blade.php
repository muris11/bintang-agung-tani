    ...
    <form action="/admin/users/{{ $user->id }}" method="POST" class="max-w-2xl">
        ...
        <div class="card p-6 space-y-6">
            ...
            <div class="flex items-center gap-4 pb-4 border-b border-gray-100">
                <img loading="lazy" src="https://ui-avatars.com/api/?name={{ $userAvatar }}&background=ecfdf5&color=059669&size=64" alt="{{ $user->name }}" class="w-16 h-16 rounded-full ring-2 ring-primary-200">
                <div>
                    <h3 class="font-bold text-gray-900">{{ $user->name }}</h3>
                    <p class="text-sm text-gray-500">Bergabung: {{ $user->created_at->locale('id')->isoFormat('D MMMM YYYY') }}</p>
                </div>
            </div>
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" class="form-input w-full" placeholder="Masukkan nama lengkap" required>
                </div>

                <div>
                    <label for="email" class="form-label mb-1.5 block">Alamat Email <span class="text-red-500">*</span></label>
                    <input type="email" id="email" name="email" value="{{ old('email', $user->email) }}" class="form-input w-full" placeholder="nama@email.com" required>
                </div>

                <div>
                    <label for="phone" class="form-label mb-1.5 block">No. Telepon <span class="text-gray-400 font-normal ml-1">(Opsional)</span></label>
                    <input type="tel" id="phone" name="phone" value="{{ old('phone', $user->phone) }}" class="form-input w-full" placeholder="08xxxxxxxxxx">
                </div>
                ...
                <div>
                    ...
                    <textarea id="address" name="address" rows="3" class="form-input w-full resize-y" placeholder="Masukkan alamat lengkap">{{ old('address', $user->address) }}</textarea>
                </div>
