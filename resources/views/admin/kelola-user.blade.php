...
<div class="max-w-7xl mx-auto space-y-6 pb-12 relative z-10 w-full px-4 sm:px-0 mt-4 md:mt-0">
    ...
    <div class="card overflow-hidden border-primary-100">
        ...
        <div class="px-6 py-3 border-b border-primary-100 flex flex-wrap gap-4 items-center justify-between bg-white">
            <span class="text-xs text-gray-500 font-medium">Menampilkan {{ $users->count() }} user</span>
        </div>
                        ...
                        <tr class="hover:bg-primary-50/10 transition-colors group">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <img loading="lazy" src="https://ui-avatars.com/api/?name={{ $userAvatar }}&background=ecfdf5&color=059669&size=40" alt="{{ $user->name }}" class="w-10 h-10 rounded-full ring-2 ring-primary-100">
                                    <div>
                                        <div class="font-bold text-gray-900 group-hover:text-primary-600 transition-colors">{{ $user->name }}</div>
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="text-gray-600 text-xs">{{ $user->created_at->locale('id')->isoFormat('D MMM YYYY') }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="/admin/users/{{ $user->id }}/edit" class="icon-button text-gray-500 hover:text-amber-600 hover:bg-amber-50 bg-white border border-gray-200 rounded-lg shadow-sm transition-colors" title="Edit">
                                        <i class="ph ph-pencil-simple ph-bold w-4 h-4"></i>
                                    </a>
                                    <form action="/admin/users/{{ $user->id }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus user ini?')">
                                        @csrf
                                        @method('DELETE')
