            ...
            <div class="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="product" placeholder="Cari produk..." value="{{ request('product') }}" 
                ...
                <div>
                    ...
                    <select name="type" class="form-input w-full">
                        ...
                        <option value="increase" {{ request('type') == 'increase' ? 'selected' : '' }}>Tambah Stok</option>
                        <option value="decrease" {{ request('type') == 'decrease' ? 'selected' : '' }}>Kurang Stok</option>
                    </select>
                </div>
            </div>
                    ...
                    @forelse($stockLogs as $log)
                        <tr class="hover:bg-primary-50/30 transition-colors">
                            <td class="px-6 py-4 text-gray-900 font-medium">
                                ...
                                <span class="text-gray-500 text-xs block">{{ $log->created_at->format('H:i') }} WIB</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="font-medium text-gray-900">{{ $log->product->name ?? '-' }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right font-bold text-gray-900">{{ $log->quantity }}</td>
                            <td class="px-6 py-4 text-right text-gray-600">{{ $log->before_stock }}</td>
                            <td class="px-6 py-4 text-right font-bold {{ $log->after_stock > $log->before_stock ? 'text-green-600' : 'text-red-600' }}">{{ $log->after_stock }}</td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <div class="w-6 h-6 rounded-full bg-primary-100 text-primary-600 flex items-center justify-center text-xs font-bold">
                                        {{ substr($log->createdBy?->name ?? 'S', 0, 1) }}
                                    </div>
                                    <span class="text-sm text-gray-700">{{ $log->createdBy?->name ?? 'System' }}</span>
                                </div>
                            </td>
                        </tr>
                    @empty
