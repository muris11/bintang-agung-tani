...
<body>
    ...
    <div class="header">
        <div class="brand">
            <h1>{{ $storeName }}</h1>
            <div class="muted">{{ $storeBranch }}</div>
            <div class="muted">{{ $storeAddressLine1 }}</div>
            <div class="muted">{{ $storeAddressLine2 }}</div>
            <div class="muted">{{ $storePhone }} | {{ $storeEmail }}</div>
        </div>
        ...
        <div class="meta">
            ...
            <div style="margin-top: 8px;">
                <span class="badge">{{ $order->getStatusLabel() }}</span>
            </div>
        </div>
    </div>
        ...
        <tr>
            <td style="padding-right: 10px;">
                <div class="panel">
                    ...
                    <div class="value">{{ $order->shipping_phone ?? $order->user->phone ?? '-' }}</div>
                </div>
            </td>
            <td style="padding-left: 10px;">
                <div class="panel">
                    ...
                    <div class="value">{{ $order->shipping_courier ?? '-' }}</div>
                    <span class="label">Layanan</span>
                    <div class="value">{{ $order->shipping_service ?? '-' }}</div>
                    <span class="label">Alamat</span>
                    <div class="value">{{ $order->shipping_address_snapshot }}</div>
                </div>
            </td>
        </tr>
    </table>
                ...
                @foreach ($order->items as $item)
                    <tr>
                        <td>
                            ...
                            <div style="color: #6b7280; font-size: 11px;">SKU: {{ $item->product_sku ?? '-' }}</div>
                        </td>
