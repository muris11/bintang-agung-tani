...
@section('title', 'QR Code Pembayaran')
...
<div class="max-w-2xl mx-auto space-y-6 pb-12">
    ...
    <div class="card p-8 animate-fade-in-up delay-100">
        <div class="text-center mb-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-1">Order #{{ $order->order_number }}</h2>
            <p class="text-3xl font-bold text-primary-600">{{ $order->getFormattedTotal() }}</p>
        </div>

        @if($order->qr_code_path && $order->getQrCodeUrl())
            <!-- QR Code Display -->
            <div class="flex flex-col items-center">
                ...
                <!-- Download Button -->
                <a href="{{ route('user.payments.download-qr', $order) }}" class="btn-secondary mt-6 flex items-center gap-2">
            ...
            @if($order->latestPaymentProof)
                <div class="mt-8 p-4 rounded-xl {{ $order->latestPaymentProof->isPending() ? 'bg-yellow-50 border border-yellow-200' : ($order->latestPaymentProof->isVerified() ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200') }}">
                    <div class="flex items-center gap-3">
                        <div class="shrink-0">
                            @if($order->latestPaymentProof->isPending())
                                <i class="ph ph-clock w-6 h-6 text-yellow-600"></i>
                            @elseif($order->latestPaymentProof->isVerified())
                        ...
                        <div>
                            <p class="font-semibold {{ $order->latestPaymentProof->isPending() ? 'text-yellow-800' : ($order->latestPaymentProof->isVerified() ? 'text-green-800' : 'text-red-800') }}">
                                {{ $order->latestPaymentProof->getStatusLabel() }}
                            </p>
                            <p class="text-sm {{ $order->latestPaymentProof->isPending() ? 'text-yellow-700' : ($order->latestPaymentProof->isVerified() ? 'text-green-700' : 'text-red-700') }}">
                                @if($order->latestPaymentProof->isPending())
                                    Admin akan memverifikasi pembayaran Anda dalam 1x24 jam
                                @elseif($order->latestPaymentProof->isVerified())
                                    Pembayaran telah diverifikasi. Pesanan sedang diproses.
                                @else
                                    Pembayaran ditolak. {{ $order->latestPaymentProof->admin_notes }}
                                @endif
                            </p>
                        </div>
                    </div>
        ...
        <div class="space-y-4">
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">Buka aplikasi {{ Setting::get('store_name', 'Bintang Agung Tani') }} di HP Anda dan scan QR code ini</p>
                </div>
            </div>
            <div class="flex items-start gap-4">
    ...
    <div class="text-center animate-fade-in-up delay-200">
        <a href="{{ route('user.orders.show', $order) }}" class="btn-secondary inline-flex items-center gap-2">
