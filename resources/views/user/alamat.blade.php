    ...
    @else
        <div class="grid gap-4">
            @foreach($addresses as $address)
                <div class="bg-white rounded-lg shadow p-6 {{ $address->is_default ? 'border-2 border-green-500' : '' }}">
                    <div class="flex justify-between items-start mb-4">
                        ...
                        <div class="flex gap-2">
                            ...
                            <form action="{{ route('user.alamat.destroy', $address) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                    ...
                    <p class="text-gray-600 mb-4">{{ $address->full_address }}</p>
                    <p class="text-gray-500 text-sm">{{ $address->city }}, {{ $address->postal_code }}</p>
                    
                    @if(!$address->is_default)
                        <form action="{{ route('user.alamat.default', $address) }}" method="POST" class="mt-4">
                            @csrf
                            @method('PATCH')
...
<div id="addAddressModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-lg max-w-lg w-full max-h-screen overflow-y-auto">
        ...
        <form action="{{ route('user.alamat.store') }}" method="POST" class="p-6">
            @csrf
            <div class="space-y-4">
                <div>
...
function editAddress(address) {
    document.getElementById('editAddressForm').action = `{{ route('user.alamat.index') }}/${address.id}`;
